...
use App\Models\{Admin\Setting, User};
use App\Models\Frontend\{Contactus, HomeContent, ProductCategory, ProductSubCategory, SubscriberEmail};
...
class HomeController extends Controller
{
    ...
    public function getFaq(){
       $array = [];
        $res=(array)Setting::where('key','faq')->pluck('long_value')->first();
       foreach ($res['title'] as $key=>$item){
           $array[]=[
               'title'=> $item,
               'caption' => $res['caption'][$key]
           ];
       }
    ...
    public function getFooter(){
        $data['featured_category'] = ProductCategory::where(['is_featured'=>1,'is_active'=>1])->select(['name','id','slug'])->get();
        $data['animation_category'] = ProductSubCategory::where(['is_featured'=>1,'is_active'=>1,'category_id'=>2])->select(['name','id','slug'])->get();
        $data['settings'] = (object)Setting::whereIn('key',[
            'facebook_url',
            'twitter_url',
            'instagram_url',
            'youtube_url',
    ...
    public function requestQuote(Request $request){
        ...
        Contactus::create(array_merge($request->all(),['type'=>'Quote']));
        /**
