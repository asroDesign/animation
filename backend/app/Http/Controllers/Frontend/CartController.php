...
class CartController extends Controller
{
    ...
    public function store(request $request) {
        ...
        $url = (@$request->buy_now == 1) ? '/dashboard/cart?step=2' : '/dashboard/cart'; //route('frontend.checkout',app()->getLocale()) : route('frontend.cart.index',app()->getLocale());
        ...
        $cartItem=Cart::add($product->id, $productName, 1, $productPrice, ['variants' => $variants])->associate('App\Models\Frontend\Product');
        \DB::table('carts')
            ->updateOrInsert(
                ['json' => json_encode([$product->id, $productName, 1, $productPrice, ['variants' => $variants]])],
                ['user_id' => auth()->id()],
            );
        return response()->json(['status' => true, 'msg' => trans('frontend_msg.prod_added_to_cart'),'url'=>$url,'cart'=>$cartItem], 200);
    }
