...
//		$obj->username  = $request->username;
	...
	function change_password(Request $request)
	{
		...
		$rules['newPassword'] = ['required', 'min:6','regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*(_|[^\w])).+$/'];
		$rules['confirmPassword'] = 'required|same:newPassword';
		$msg['old_password.required'] = trans('frontend_msg.req_old_password');
		$msg['newPassword.required'] = trans('frontend_msg.new_password');
		$msg['newPassword.min'] = trans('frontend_msg.req_password_min');
		$msg['newPassword.regex'] = trans('frontend_msg.invalid_password_format');
		$msg['confirmPassword.required'] = trans('frontend_msg.req_confirmPassword');
		$msg['confirmPassword.same'] = trans('frontend_msg.password_confirmed_not_match');
		...
		if (!Hash::check($request->old_password, $user->newPassword)){// CHECK OLD PASSWORD
	...
	function update_user_image(Request $request)
	{
        ...
        $rules['image.*'] = 'mimes:jpeg,png,jpg,gif,svg';
		...
		$obj = Auth::user();
		$obj->save();
		return json_response(['status' => true, 'msg' => trans('frontend_msg.succ_update_user_image')], 200);
	}
