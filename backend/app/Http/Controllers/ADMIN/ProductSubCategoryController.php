...
class ProductSubCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('banner')){
            $obj->banner_image = $request['banner']->store($obj->previewfolderPath());
        }
        if ($request->hasFile('video_file')){
            $obj->file = $request['video_file']->store($obj->previewfolderPath());
//            if ( in_array($request['video_file']->extension(),['mpeg','mkv','mp4','gif'])){
//                FFMpeg::fromDisk('public')
//                    ->open($obj->previewfolderPath())
//                    ->export()
//                    ->inFormat(new WebM())
//                    ->save($obj->filefolderPath().'/'.$request['video_file']->getFilename());
//            }
        }
...
//        dd(ProductSubCategory::find($id)['features']);
        if(empty($data)){
