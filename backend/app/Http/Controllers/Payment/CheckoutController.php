...
use App\Models\{Order, OrderProduct, Setting, Wallet};
...
class CheckoutController extends Controller
{
    ...
    {
        $old_cart = \DB::table('carts')->where('user_id',auth()->id())->first();
        if ($old_cart){
            $e=(object)json_decode($old_cart->json);
            Cart::add($e->id,$e->name,$e->qty,$e->price,(array)$e->options)->associate('App\Models\Frontend\Product');
        }
        if (Cart::instance('default')->count() > 0) {  //if the cart has items
            ...
            $discount = $e->coupon->discount ?? session('coupon')['discount'] ?? 0;
            ...
            return response()->json([
                ...
                'discount_code'=> $e->coupon->code ?? session('coupon')['code'] ?? "",
                'setting' => getsetting(),
        ...
        return response()->json([
            'msg' => 'You have nothing in your cart , please add some products first'
        ],404);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $discount = session('coupon')['discount'] ?? $e->coupon->discount  ?? 0;
            ...
            switch ($request->gateway){
                ...
                case "free":
                    if($subtotal > 0){
                        ...
                        return response()->json([
                            'msg' => trans('frontend_msg.something_went_wrong')
                        ],403);
                    }
				...
				return response()->json([
                    'msg' => trans('frontend_msg.payment_type_not_found')
                ],403);
		    }
            return response()->json([
                'msg' => trans('frontend_msg.payment_type_not_found')
