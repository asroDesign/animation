...
<div class="tp_main_content_wrappo">
    <div class="tp_tab_wrappo">
        <ul>
            <li class="active"><a href="{{ route('admin.processes.index') }}">Manage Project Process</a></li>
        </ul>
    </div>
                                ...
                                @if ($data->count() > 0)
                                    @foreach ($data as $key => $item)
                                        <tr id="table_row_{{ $item->id }}">
                                            ...
                                            <td>{{ $item->current_step_id }} </td>
                                            <td>
                                                <div class="tp_autoresponder">
                                                    <label class="tp_toggle_label">
                                                        <input id="active_btn_{{ $item->id }}" type="checkbox"
                                                            name="cate_status" value="1"
                                                            onclick="update_single_status('{{ route('processes.update', $item->id) }}','{{ $item->is_active }}','{{ 'active_btn_' . $item->id }}')"
                                            ...
                                            <td>
                                                <ul>
                                                    <li><a href="{{ route('admin.processes.show', $item->id) }}"
...
{{--                                                            onclick="delete_single_details('{{ route('admin.processes.destroy', $item->id) }}',{{ $item->id }})"><i--}}
